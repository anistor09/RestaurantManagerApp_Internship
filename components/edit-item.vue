...
async function addAiDescription(neededLength: string, short: boolean) {
    if (name.value.length === 0) {
        ...
        if (short) { description.value = "The new description is loading..." }
        else { longDescription.value = "The new description is loading..." }
        const requestBody = {
            itemName: name.value,
            length: neededLength,
            target: 'an item',
        ...
        if (short) { description.value = response.data.value }
        else { longDescription.value = response.data.value }
    }
}
                        ...
                        <el-scrollbar>
                            <el-col>
                                <el-row v-for="option in options" :key="option.id" class="menus-row">
                                    <option-component :id="option.id" :option-name="option.name" :choices="option.choiceSet"
                                        :options="options" @delete-choice="handleDeleteChoiceEmit"
                                        @delete-option="handleDeleteOptionEmit" @edit-choice="handleEditChoiceEmit"
                    ...
                    <div class="box" style="justify-content: center;">
                        ...
                        <el-select v-model="selectedAllergens" class="special-select-item" multiple collapse-tags filterable
                            ...
                            <el-option v-for="allergenOption in allergens" :key="allergenOption.id"
                                :label="allergenOption.name" :value="allergenOption.name" />
                        </el-select>
                    </div>
                </div>
                <div class=elementLeft>
    ...
    <ClientOnly>
        ...
        <Teleport to="body">
            <el-dialog v-model="delteteOptionPopUp" class="choice-delete-popup">
                <div
                    style="width: 100%;height: 70%;text-align: center; display: flex; justify-content: center; align-items: center;">
